...
def process_document(uploaded_file: UploadedFile) -> list[Document]:
    ...
    temp_file.close()  # Close the file to ensure it's not being used
...
def add_to_vector_collection(all_splits: list[Document], file_name: str):
    ...
    # Log the documents and their embeddings
    # st.write("Documents to be upserted:", documents)
    collection.upsert(
        documents=documents,
        metadatas=metadatas,
        ids=ids,
...
def upload_file():
    ...
    if file:
        ...
        add_to_vector_collection(all_splits, file.filename)
        return jsonify({"message": "File processed and data added to the vector store."}), 200
...
@app.route('/re_rank', methods=['POST'])
def re_rank():
    data = request.get_json()
    documents = data.get('documents')
    relevant_text, relevant_text_ids = re_rank_cross_encoders(documents)
    return jsonify({"relevant_text": relevant_text, "relevant_text_ids": relevant_text_ids}), 200
